...
const Cart = () => {
    ...
    const updatedCart = cartItems.map((item) => {
      if (item.batchId === batchId) {
        ...
        if (quantity !== item.quantity) {
          toast.success(`Updated ${item.name} quantity to ${quantity}`, { autoClose: 1500 });
        }
        return { ...item, quantity, totalWithGST };
      }
      return item;
  ...
  const handleRemoveItem = (batchId) => {
    const itemToRemove = cartItems.find((item) => item.batchId === batchId);
    const updatedCart = cartItems.filter((item) => item.batchId !== batchId);
    setCartItems(updatedCart);
    localStorage.setItem("cart", JSON.stringify(updatedCart));
    window.dispatchEvent(new Event("cartUpdated"));
    toast.info(`Removed ${itemToRemove.name} from cart`, { autoClose: 2000 });
  };

  const handleCheckout = () => {
    if (cartItems.length === 0) {
      toast.warn("Your cart is empty! Add items to proceed.", { autoClose: 2000 });
      return;
    }
    navigate("/checkout");
  };
      ...
      <Box sx={{ display: "flex", flexDirection: "column", minHeight: "100vh", bgcolor: "background.default" }}>
        ...
        >
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
            <Typography
              variant={isMobile ? "h5" : "h4"}
              sx={{ color: "primary.main", fontWeight: 700, fontSize: { xs: "1.5rem", sm: "1.75rem", md: "2.5rem" } }}
            >
            ...
            {cartItems.length > 0 && (
              <Button
                ...
                color="secondary"
                onClick={clearCart}
                sx={{ color: "#D32F2F", borderColor: "#D32F2F", "&:hover": { bgcolor: "#FF6655", color: "#FFFFFF" } }}
              >
                Clear Cart
              </Button>
            )}
                      ...
                      <TableBody>
                        {cartItems.map((item, index) => (
                          <motion.tr
                            ...
                            sx={{ "&:hover": { bgcolor: "#F1F8E9" }, transition: "background-color 0.3s ease" }}
                          >
                            <TableCell sx={{ minWidth: { xs: 150, sm: 200 } }}>
                              <Box sx={{ display: "flex", alignItems: "center" }}>
                                {item.image && (
