...
//       const res = await axios.get("${API_BASE_URL}/api/services/list", {
...
//     try {
//       const res = await axios.get(`${API_BASE_URL}/api/bookings/customer/${phoneNumber}`, { withCredentials: true });
...
//         return axios.post("${API_BASE_URL}/api/book-service", bookingData, { withCredentials: true });
//       });
...
const AdminOfflineService = () => {
  ...
  const fetchServices = async () => {
    ...
    setError("");
    try {
      const res = await axios.get("${API_BASE_URL}/api/services/list", { withCredentials: true });
      setServices(res.data || []);
    } catch (error) {
      console.error("Error fetching services:", error.response?.data || error.message);
      setServices([]);
