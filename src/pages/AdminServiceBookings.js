...
const AdminServiceBookings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const userRes = await axios.get("${API_BASE_URL}/api/users/me", { withCredentials: true });
        ...
        const [bookingsRes, laborsRes] = await Promise.all([
          axios.get("${API_BASE_URL}/api/bookings/all", { withCredentials: true }),
          axios.get("${API_BASE_URL}/api/labors/list", { withCredentials: true }),
        ]);
        setBookings(
  ...
  const handleApprove = async (bookingId) => {
    ...
    try {
      const response = await axios.put(
        `${API_BASE_URL}/api/bookings/approve/${bookingId}`,
