...
//         await axios.delete(`${API_BASE_URL}/api/users/${customerId}`, {
...
const ManageCustomer = () => {
  ...
  const fetchCustomers = async () => {
    setLoading(true);
    try {
      const response = await axios.get("${API_BASE_URL}/api/users/customers", { withCredentials: true });
      setCustomers(response.data.customers);
    } catch (error) {
  ...
  const handleDeleteCustomer = async () => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/users/${customerToDelete._id}`, { withCredentials: true });
