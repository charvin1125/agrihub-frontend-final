...
//     setLoading(true);
//     try {
//       const res = await axios.get("${API_BASE_URL}/api/product/list", { withCredentials: true });
...
//     try {
//       const res = await axios.get(`${API_BASE_URL}/api/orders/customer/${phoneNumber}`, { withCredentials: true });
...
//         "${API_BASE_URL}/api/orders/offline",
//         orderData,
...
const AdminOfflinePurchase = () => {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const response = await axios.post("${API_BASE_URL}/api/orders/offline", orderData, { withCredentials: true });
      if (response.data.success) {
        alert("Purchase successful!");
