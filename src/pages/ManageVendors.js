...
//     try {
//       const response = await axios.get("${API_BASE_URL}/api/vendor/list", { withCredentials: true });
...
//     try {
//       if (isEditing) {
//         await axios.put(`${API_BASE_URL}/api/vendor/${editId}`, vendor, { withCredentials: true });
//         alert("Vendor updated successfully!");
//         setIsEditing(false);
//         setEditId(null);
//       } else {
//         await axios.post("${API_BASE_URL}/api/vendor/add", vendor, { withCredentials: true });
...
//       try {
//         await axios.delete(`${API_BASE_URL}/api/vendor/${id}`, { withCredentials: true });
...
//     }
//     try {
//       await axios.delete(`${API_BASE_URL}/api/vendor/${vendorToDelete}`, { withCredentials: true });
...
//       setFilteredVendors(response.data);
//     } catch (error) {
//       toast.error("Failed to fetch vendors", { position: "top-center" });
//     }
//   };
...
//   const validateForm = () => {
//     let tempErrors = {};
//     if (!vendor.name.trim()) tempErrors.name = "Company Name is required";
...
//     }
//     if (!vendor.address.trim()) tempErrors.address = "Address is required";
...
//     e.preventDefault();
//     if (!user?.isAdmin) {
//       toast.error("Unauthorized: Admin access only", { position: "top-center" });
//       return;
//     }
...
//       } else {
//         await axios.post("${API_BASE_URL}/api/vendor/add", vendor, { withCredentials: true });
//         toast.success("Vendor added successfully!", { position: "top-center" });
//       }
//       setVendor({ name: "", email: "", phone: "", address: "" });
//       fetchVendors();
//     } catch (error) {
...
//   const handleDelete = async () => {
//     if (!user?.isAdmin) {
//       toast.error("Only admins can perform this action", { position: "top-center" });
//       return;
//     }
//     try {
//       await axios.delete(`${API_BASE_URL}/api/vendor/${vendorToDelete}`, { withCredentials: true });
...
//       setVendorToDelete(null);
//     } catch (error) {
//       toast.error("Failed to delete vendor", { position: "top-center" });
//     }
//   };
...
//     link.click();
//     window.URL.revokeObjectURL(url);
//     toast.success("Vendors exported successfully!", { position: "top-center" });
//   };
...
//       MuiTableCell: {
//         styleOverrides: {
//           head: { fontWeight: "600", fontSize: "0.75rem", color: darkMode ? "#b0b0b0" : "#757575" },
//           body: { fontSize: "0.875rem", color: darkMode ? "#e0e0e0" : "#212121" },
//         },
//       },
//       MuiButton: {
//         styleOverrides: {
//           root: { borderRadius: "8px", textTransform: "none", padding: "6px 12px" },
//           contained: { boxShadow: "none", "&:hover": { backgroundColor: "#388E3C" } },
//         },
//       },
//       MuiTextField: {
//         styleOverrides: {
...
//               {user?.isAdmin && (isEditing || !editId) && (
//                 <Card sx={{ mb: 4, bgcolor: "background.paper", borderRadius: "10px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
//                   <CardContent>
//                     <Typography variant="h6" sx={{ mb: 3, color: "text.primary", fontWeight: "medium" }}>
//                       {isEditing ? "Edit Vendor" : "Add New Vendor"}
//                     </Typography>
//                     <form onSubmit={handleSubmit}>
//                       <Grid container spacing={2}>
...
//               <TableContainer component={Paper} sx={{ borderRadius: "10px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
...
//                   <TableBody>
//                     {currentVendors.map((v) => (
//                       <TableRow key={v._id} sx={{ "&:hover": { bgcolor: darkMode ? "#2a2a2a" : "#f5f5f5" } }}>
//                         <TableCell>
//                           <Checkbox
//                             checked={selectedVendors.includes(v._id)}
//                             onChange={() => handleSelectVendor(v._id)}
...
//                         {user?.isAdmin && (
//                           <TableCell align="right">
//                             <IconButton onClick={() => handleEdit(v._id, v)}>
//                               <EditIcon />
//                             </IconButton>
//                             <IconButton onClick={() => openDeleteDialog(v._id)}>
//                               <DeleteIcon />
//                             </IconButton>
//                           </TableCell>
//                         )}
...
//             <Dialog open={deleteDialogOpen} onClose={closeDeleteDialog} PaperProps={{ sx: { borderRadius: "10px" } }}>
...
//                   <Link to="/admin-dashboard" style={{ textDecoration: "none", color: "#4CAF50", display: "flex", alignItems: "center" }}>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        await Promise.all([fetchVendors(), fetchUser()]);
      } finally {
    ...
    handleResize();
    return () => window.removeEventListener("resize", handleResize);
  }, [navigate]); // Added navigate to dependency array since it's used in fetchUser
