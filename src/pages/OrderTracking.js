...
//       try {
//         const res = await axios.get("${API_BASE_URL}/api/orders/my-orders", { withCredentials: true });
...
const OrderTracking = ({ darkMode = false }) => {
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 600);
              ...
              <Grow in timeout={500 * (index + 1)} key={order._id}>
                <Card sx={{ mb: 3 }}>
                  <CardContent sx={{ p: { xs: 2, sm: 2.5, md: 3 } }}>
                    ...
                    <List dense sx={{ bgcolor: darkMode ? "#2A2A2A" : "#F7F9F7", borderRadius: "10px", p: 1 }}>
                      {(order.cart || []).map((item, idx) => (
                        <ListItem
                          key={idx}
                          sx={{
