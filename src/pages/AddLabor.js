...
const ManageLabor = () => {
  ...
  const fetchLabors = async () => {
    try {
      const res = await axios.get("${API_BASE_URL}/api/labor/list", { withCredentials: true });
    ...
    try {
      if (editId) {
        const res = await axios.put(
          `${API_BASE_URL}/api/labor/edit/${editId}`,
  ...
  const handleDeleteLabor = async (id) => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/labor/delete/${id}`, { withCredentials: true });
