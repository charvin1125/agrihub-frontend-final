...
// // Define constants at the top to avoid initialization error
// const FOOTER_START = "#B3D8A8"; // Matching Footer's soft green
// const FOOTER_END = "#3D8D7A";   // Matching Footer's teal
...
//         styleOverrides: {
//           root: {
//             background: `linear-gradient(135deg, ${FOOTER_START} 0%, ${FOOTER_END} 100%)`, // Matching Footer's gradient
//             boxShadow: "none",
//           },
//         },
//       },
...
import axios from "axios";
import { createTheme, ThemeProvider } from "@mui/material/styles";
import { ToastContainer, toast } from "react-toastify"; // Added for error feedback
...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || "http://localhost:5000"; // Fallback for development

const FOOTER_START = "#B3D8A8";
const FOOTER_END = "#3D8D7A";
  ...
  return (
    <ThemeProvider theme={theme}>
      <AppBar position="fixed">
        ...
        >
          {isMobile && (
            <IconButton edge="start" onClick={toggleDrawer(true)} sx={{ mr: 1, p: 0.75 }}>
              <MenuIcon sx={{ fontSize: { xs: "2rem", sm: "2.25rem" }, color: "#1A1A1A" }} />
            </IconButton>
          )}
          ...
          {isMobile && (
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
              <IconButton component={Link} to="/cart" sx={{ p: 0.75 }}>
                <Badge badgeContent={cartCount} color="error">
                  <ShoppingBagIcon sx={{ fontSize: "1.75rem" }} />
                </Badge>
              </IconButton>
              <IconButton onClick={handleMenuOpen} sx={{ p: 0.75 }}>
                <AccountCircleIcon sx={{ fontSize: "1.75rem" }} />
              </IconButton>
            </Box>
          )}
